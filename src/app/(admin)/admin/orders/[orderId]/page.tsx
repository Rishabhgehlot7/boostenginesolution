...
export default function OrderDetail() {
  ...
  const handleReturnStatusChange = async (returnRequestId: string) => {
    try {
      const response = await axios.post(
        `/api/returnOrder/UpdateReturnStatus/`,

        { id: returnRequestId, returnStatus: returnStatus }
      );
      fetchOrder();
      toast({
      ...
      toast({
        ...
        description: "An error occurred while updating the return status.",
        variant: "destructive",
      });
    }
  };
                ...
                {order.status === "returned" ? (
                  <div className="mb-4">
                    ...
                    <div className="mb-4">
                      ...
                      <p>Return Status: {order.status}</p>
                      <div className="mt-2">
                        <select
                          value={returnStatus}
                        ...
                        <Button
                          onClick={() => handleReturnStatusChange(order._id)}
                          className="ml-2 bg-white text-black"
                        >
